...
export async function POST(req: NextRequest) {
  ...
  try {
    // Upsert in case this order was created before we subscribed to orders/create.
    await sql`
      INSERT INTO shopify_orders (
        id, order_number, customer_email, total_price, currency,
        financial_status, fulfillment_status, source_name, tags, is_vip,
        assigned_rep, service_type, raw_payload, shopify_created_at
      ) VALUES (
        ${order.id}, ${String(order.order_number)}, ${order.email},
